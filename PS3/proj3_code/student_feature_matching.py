...
def match_features(features1, features2, x1, y1, x2, y2):
    ...
    for i in range(dists.shape[0]):
        n=dists[i]
        i1, i2 = np.argpartition(n,1)[0:2]
        d1=n[i1]
        d2=n[i2]
        ratio=d1/d2
        if(ratio <0.8):
            matches.append([i,i1])
            confidences.append(d1)
    confidences=np.array(confidences)
    matches= np.array(matches)
    idx= np.argsort(confidences)
    confidences=confidences[list(idx)]
    matches=matches[idx]
...
def pca(fvs1, fvs2, n_components= 24):
    ...
    #############################################################################
    data=np.vstack((fvs1, fvs2))
    data= data-np.mean(data,axis=0)
    cov1=np.cov(data, rowvar=False)
    w, v = np.linalg.eig(cov1)
    idx= np.argpartition(-w,n_components)[:n_components]
    v=v.T[idx]
    reduced_fvs1=np.dot(data,v.T)[:fvs1.shape[0]]
    reduced_fvs2=np.dot(data,v.T)[fvs1.shape[0]:]    
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
