...
def get_feat_vec(x,y,magnitudes, orientations,feature_width):
    ...
    for i in range(4):
        for j in range(4):
            temp_mag=mag[flq*i:flq*i+flq,flq*j:flq*j+flq]
            temp_ori=ori[flq*i:flq*i+flq,flq*j:flq*j+flq]
            temp_mag=temp_mag.flatten()
            temp_ori=temp_ori.flatten()
            temp_ori_idx= np.floor(((temp_ori/(np.pi/8))+8)/2)
            for o in range(len(temp_ori_idx)):
                oi= int(temp_ori_idx[o])
                if(oi==8):
                    oi=7
                fv[i][j][oi]+=temp_mag[o]
    fv=fv.flatten()
...
def get_features(image, x, y, feature_width):
    ...
    for i in range(len(x)):
        xi=x[i]
        yi=y[i]
        fv=get_feat_vec(xi,yi,magnitudes, orientations,feature_width)
        fvs.append(fv)
    fvs=np.array(fvs)
    #############################################################################
    #                             END OF YOUR CODE                              #
