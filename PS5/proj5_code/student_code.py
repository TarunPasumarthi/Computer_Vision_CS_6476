...
def pairwise_distances(X, Y):
    ...
    for n in range(N):
        row=[]
        for m in range(M):
            row.append(np.sqrt(np.sum((Y[m]-X[n])**2)))
        D.append(row)
    D=np.array(D)
    #############################################################################
    #                             END OF YOUR CODE
...
def nearest_neighbor_classify(train_image_feats,
    ...
    for y in range(D.shape[1]):
        xd= D[:,y]
        idx=np.argpartition(xd, k)[:k]
        labels=tl[idx]
        lab=stats.mode(labels)[0][0]
...
def kmeans(feature_vectors, k, max_iter=100):
    ...
    #  #
    #############################################################################
    centers_idx= np.random.choice(feature_vectors.shape[0],k,replace=False)
    centroids=feature_vectors[centers_idx]
    for j in range(max_iter):
        ...
        for i in range(centroids.shape[0]):
            ...
            cluster= feature_vectors[idx]
            if len(cluster)==0:
                marg= np.argmax(dist[:,i])
                center=feature_vectors[marg]
            else:
...
def build_vocabulary(image_arrays, vocab_size=50, stride=20, max_iter=10):
    ...
    *   The default vocab_size of 50 is sufficient for you to get a
    ...
    #############################################################################
    sift_arr=[]
    ilen=len(image_arrays)
    for img in image_arrays:
        h=img.shape[0]
        w=img.shape[1]
        img_torch=torch.from_numpy(img)
        img_torch=img_torch.view(1,1,h,w)
        img_torch= img_torch.type(torch.float32)
        xs, ys= generate_sample_points(h,w,stride)
        feats= get_siftnet_features(img_torch,xs,ys)
        for f in feats:
            sift_arr.append(f)
    sift_arr= np.array(sift_arr)
    vocab= kmeans(sift_arr,vocab_size,max_iter)
    #############################################################################
    #                             END OF YOUR CODE
    #                             #
    #############################################################################
...
def kmeans_quantize(raw_data_pts, centroids):
    ...
    # TODO: YOUR CODE HERE
    #############################################################################
    dist= pairwise_distances(raw_data_pts, centroids)
...
def get_bags_of_sifts(image_arrays, vocabulary, stride=5):
    ...
    for n, img in enumerate(image_arrays):
        img=np.array(img)
        h=img.shape[0]
        w=img.shape[1]
        img_torch=torch.from_numpy(img)
        ...
        img_torch= img_torch.type(torch.float32)
        xs, ys= generate_sample_points(h,w,stride)
        sfeats= get_siftnet_features(img_torch,xs,ys)
        for f in sfeats:
            f=np.array([f])
            dist=pairwise_distances(f, vocab)
            idx=np.argmin(dist)
            feats[n,idx]+=1
    feats=feats.T
