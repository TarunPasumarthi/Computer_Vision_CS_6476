...
class HistogramLayer(nn.Module):
    ...
    def forward(self, x) -> torch.Tensor:
        ...
        ** You will receive extra-credit if you successfully implement this
        ...
        Returns:
        -   per_px_histogram: tensor with shape (1,8,M,N) representing a
        weighted
        ...
        if method == 'triple_for_loop':
            ...
            dim1_idxs = bin_idxs.flatten()  # max in each channel, at each (
            # x,y) location
...
class SubGridAccumulationLayer(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        self.layer = nn.Conv2d(in_channels=8, out_channels=8, kernel_size=4,
                               bias=False, padding=(2, 2),
                               padding_mode='zeros', groups=8)
...
class SIFTOrientationLayer(nn.Module):
    ...
    def get_orientation_bin_weights(self) -> torch.nn.Parameter():
        ...
        # since cosine similarity bin would be from [-pi/8,pi/8], we shift
        # it to [0,pi/4]
        theta_edges += np.pi / 8
        edge_vectors = angles_to_vectors_2d_pytorch(theta_edges).type(
            torch.FloatTensor)
...
class SIFTNet(nn.Module):
    ...
    def __init__(self):
        ...
        # (1) Conv layer to compute image gradients with two directed Sobel
        # filters.
        # (2) Conv layer to project image gradients along 8 basis
        # directions, and to
        #   replicate the gradient values for the subsequent histogram
        #   computation.
        # (3) HistogramLayer
...
def get_siftnet_features(img_bw: torch.Tensor, x: np.ndarray,
    ...
    histogram_grids_per_px = histogram_grids_per_px.detach().squeeze().permute(
        1, 2, 0)
    ...
    for i, (x_center, y_center) in enumerate(zip(x, y)):
        ...
        fvs[i, :] = histogram_grids_per_px[y_subgrid, x_subgrid,
                    :].flatten().numpy()
